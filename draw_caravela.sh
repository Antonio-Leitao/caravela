...
s=$(ps -p $ppid -oetime= | tr '-' ':' | awk -F: '{ total=0; m=1; } { for (i=0; i < NF; i++) {total += $(NF-i)*m; m *= i >= 2 ? 24 : 60 }} {print total}')
clean_uptime
...
#EACH LINE MUST(!!) HAVE A SPACE IN THE END
info=(
    " " 
    " " 
    " " 
    " ( /\ /? /\ \/ [- |_ /\ " 
    "  ------------ " 
    " ${BOLD}${BLUE}VERSION:${NORMAL}$version" 
    " ${BOLD}${BLUE}STUFF : ${NORMAL}$preffer" 
    " ${BOLD}${BLUE}MORE STUFF : ${NORMAL}Some actually long text over here" 
    " ${BOLD}${BLUE}SAILING FOR : ${NORMAL}$uptime" 
    " ${BOLD}${BLUE}PREFER : ${NORMAL}$preffer" 
    " ${BOLD}${BLUE}DATE : ${NORMAL}Today" 
    " ${BOLD}${BLUE}CREATED BY : ${NORMAL}Antonio Leitao" 
    " ${BOLD}${BLUE}URL : ${NORMAL}https://github.io/Antonio-Leitao/caravela" 
    " " 
    " ${NORMAL}$(whoami)@$(hostname)" 
    " ${BOLD}${BLUE}OS : ${NORMAL}$distro" 
    " " 
    " " 
    " " 
    " " 
